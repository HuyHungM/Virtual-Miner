...
export async function executeShop(
    ...
    const containerComponent = new ContainerBuilder().setAccentColor(
    ...
    const balanceComponent = new TextDisplayBuilder().setContent(
        `Số dư: **$${user.balance}**`,
    );
    ...
    for (const pickaxe of result.pickaxes) {
        ...
        const textComponent = new TextDisplayBuilder().setContent(
            [`### ${emoji} **${pickaxe.name}**`, pickaxe.description].join(
                '\n',
            ),
        );

        const buttonCompontent = new ButtonBuilder()
            .setCustomId(`shop:select:${pickaxe.id}`)
            .setLabel(
                `${owned ? (pickaxe.id === result.currentPickaxe?.id ? 'Đã trang bị' : 'Trang bị') : `$${pickaxe.price}`}`,
            )
            .setEmoji(client.appEmojis.get(pickaxe.emoji) ?? '⛏️')
            .setStyle(owned ? ButtonStyle.Success : ButtonStyle.Primary)
            .setDisabled(!buyable || pickaxe.id === result.currentPickaxe?.id);
    ...
    const pageComponent = new TextDisplayBuilder().setContent(
