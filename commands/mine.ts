...
export async function executeMine(client: Client, interaction: any) {
    ...
    if (interaction.isButton()) {
        ...
        const pickaxeEmoji = getEmoji(client, pickaxe?.emoji ?? EMOJI_PICKAXE);
        const loading = new ContainerBuilder().addTextDisplayComponents(
            new TextDisplayBuilder().setContent(`${pickaxeEmoji} Đang đào…`),
        );
    ...
    try {
        ...
        if (!result.success) {
            switch (result.reason) {
                ...
                case 'MINING_COOLDOWN': {
                    ...
                    const pickaxeEmoji =
                        client.resources.pickaxes.get(user.pickaxe)?.emoji ??
                        EMOJI_PICKAXE;
                    ...
                    const container = new ContainerBuilder()
                        ...
                        .addTextDisplayComponents(
                            new TextDisplayBuilder().setContent(
                                `### ${getEmoji(client, EMOJI_PICKAXE)} Mining Cooldown`,
                            ),
                        )
                        ...
                        .addTextDisplayComponents(
                            new TextDisplayBuilder().setContent(
                                `Bạn cần phải chờ **${remaining}** để tiếp tục đào.`,
                            ),
                        )

                        .addActionRowComponents(
                            createButtons(client, pickaxeEmoji),
                        );
        ...
        if (trap && trap.kind === 'stun' && trap.trap?.durationMinutes) {
            ...
            trap &&
            trap.kind === 'mining_slow' &&
            trap.trap?.slowPercent
        ) {
        ...
        if (result.totalXp > 0) {
            description.push(
                `***Tổng: +${result.totalXp.toLocaleString()} XP***`,
            );
        }
        ...
        const container = new ContainerBuilder()
            ...
            // Button
            .addActionRowComponents(
                createButtons(client, result.pickaxe.emoji),
            );
        ...
        console.error('Mine command failed:', error);
        ...
        await interaction
            ...
            .catch(() => {
                // Interaction may have expired; never let this crash the bot.
            });
    }
}
