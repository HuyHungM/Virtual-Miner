...
import { pickRandom, randomInt, shuffle } from '../../shared/utils/random';
import {
    addXp,
    getRequiredXp,
    interpolate,
...
import { getUser, updateQuests } from '../user/UserService';
 ...
 * the same transaction, `claimed` marked before saving). The return value
 * carries the newly completed quests so the caller can surface a
 * notification; this module never touches Discord UI.
 */
...
export function rollDailyQuestGems(): number {
    return randomInt(QUEST_GEM.min, QUEST_GEM.max);
}
